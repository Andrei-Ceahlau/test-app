...
app.put('/api/persons/:id', async (req, res) => {
  try {
    ...
    const result = await db.Person.findByPk(id, {
      include: {
        model: db.Car,
        through: { attributes: [] }
      }
...
app.post('/api/cars', async (req, res) => {
  try {
    const { marca, model, an_fabricatie, capacitate_cilindrica, taxa_impozit } = req.body;
    
    if (!marca || !model || !an_fabricatie || !capacitate_cilindrica) {
      return res.status(400).json({ message: 'All fields are required' });
    }
    
    let calculatedTax = 50;
    ...
    const car = await db.Car.create({
      ...
      taxa_impozit: calculatedTax
    });
    
    const result = await db.Car.findByPk(car.id, {
      include: {
        model: db.Person,
        through: { attributes: [] }
      }
...
app.put('/api/cars/:id', async (req, res) => {
  try {
    ...
    const result = await db.Car.findByPk(id, {
      include: {
        model: db.Person,
        through: { attributes: [] }
      }
